...
def faq():
    st.markdown(
        """
# FAQ
## How does BlackPeakTechGPT work?
...
When you ask a question, BlackPeakTechGPT will search through the
document chunks and find the most relevant ones using the vector index.
Then, it will use GPT3 to generate a final answer.

## Is my data safe?
Yes, your data is safe. BlackPeakTechGPT does not store your documents or
...
No, the answers are not 100% accurate. BlackPeakTechGPT uses GPT-3 to generate
answers. GPT-3 is a powerful language model, but it sometimes makes mistakes 
and is prone to hallucinations. Also, BlackPeakTechGPT uses semantic search
...
But for most use cases, BlackPeakTechGPT is very accurate and can answer
